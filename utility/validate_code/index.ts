import { oneToTwoString } from '../format';
...
export async function createCode(codeBase: string) {
  const unixNow = timestampNow();
  const base = oneToTwoString(codeBase.toString()) ;
  ...
  const result =
    base +
    oneToTwoString(code1.toString()) +
    oneToTwoString(code2.toString()) +
    oneToTwoString(code3.toString()) +
    hashdata() +
    unixNow.toString()[4] +
    unixNow.toString()[5] +
    unixNow.toString()[6] +
...
export async function validateCode(code: string, exptime: number) {
  const exp = exptime;
  ...
  const validExp = codeTimeNum + exp > timeNum;
